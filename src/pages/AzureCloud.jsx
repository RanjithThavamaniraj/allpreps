...
export default function AzureCloud() {
  ...
  const azureQuestions = useMemo(() => {
    return QUESTIONS_DATA.filter(q => q.category === 'azure').map(q => ({
      id: `azure-${q.id}`,
      title: q.title,
      difficulty: q.difficulty,
      description: q.title + ' - Scenario-based question covering Azure architecture, RDS databases, IAM security, VPC networks, and S3 storage.',
      details: q.answer,
      solution: q.command,
        ...
        <section className="hero-bg" style={{ padding: '60px 0 40px', borderBottom: '1px solid var(--border)' }}>
          <div className="container" style={{ textAlign: 'left' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
              <div className="tech-card-icon-wrap" style={{ width: '48px', height: '48px', fontSize: '24px', backgroundColor: 'rgba(0, 120, 212, 0.1)', color: '#0078D4' }}>
                <VscAzure />
              </div>
