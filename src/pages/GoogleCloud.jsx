...
import { FiArrowLeft, FiSearch, FiChevronDown, FiChevronUp, FiCpu, FiCloud, FiTrendingUp } from 'react-icons/fi';
...
export default function GoogleCloud() {
  ...
  const googleQuestions = useMemo(() => {
    return QUESTIONS_DATA.filter(q => q.category === 'google').map(q => ({
      id: `google-${q.id}`,
      title: q.title,
      difficulty: q.difficulty,
      description: q.title + ' - Scenario-based question covering Google Cloud architecture, RDS databases, IAM security, VPC networks, and S3 storage.',
      details: q.answer,
      solution: q.command,
  ...
  }, [googleQuestions, searchQuery, difficultyFilter]);
        ...
        <section className="hero-bg" style={{ padding: '60px 0 40px', borderBottom: '1px solid var(--border)' }}>
          <div className="container" style={{ textAlign: 'left' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
              <div className="tech-card-icon-wrap" style={{ width: '48px', height: '48px', fontSize: '24px', backgroundColor: 'rgba(66, 133, 244, 0.1)', color: '#4285F4' }}>
                <SiGooglecloud />
              </div>
              <h1 style={{ fontSize: '36px', fontWeight: '800' }}>Google Cloud Track</h1>
            </div>
            <p className="hero-sub" style={{ margin: '0', maxWidth: '800px' }}>
              Master Google Cloud administration and design. Prepare for solutions architect and cloud operations interviews with scenario-based questions covering RDS Multi-AZ failovers, secure VPC designs, IAM roles, S3 lifecycle operations, and database migration pathways.
            </p>
          </div>
        </section>
          ...
          <div className="container">
            <div className="grid-3">
              <div className="card-saas" style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 20px' }}>
                ...
                <div>
                  <div style={{ fontSize: '20px', fontWeight: '700' }}>{googleQuestions.length} Scenario Units</div>
          ...
          <div className="container">
            <div className="questions-header-block" style={{ marginBottom: '24px' }}>
              <div className="hero-search-wrap" style={{ flex: '1', maxWidth: '480px' }}>
                ...
                <input
                  type="text"
                  placeholder="Filter Google Cloud questions..."
                  value={searchQuery}
            ...
            <div className="question-list-stack">
              {filteredQuestions.length === 0 ? (
                <div className="empty-questions">
                  <h3>No Google Cloud questions found matching the criteria</h3>
